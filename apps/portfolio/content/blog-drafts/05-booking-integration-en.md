# Building a Professional Booking System with Cal.com and Calendly in Next.js

*A comprehensive guide to integrating scheduling solutions into your portfolio*

**Reading time:** ~20 minutes  
**Difficulty:** Intermediate  
**Prerequisites:** Next.js basics, React hooks, TypeScript
...
2. [Choosing Between Cal.com and Calendly](#choosing-between-calcom-and-calendly)
3. [Setting Up Cal.com Integration](#setting-up-calcom-integration)
4. [Setting Up Calendly Integration](#setting-up-calendly-integration)
5. [Creating Reusable Booking Components](#creating-reusable-booking-components)
6. [Building a Unified Booking Page](#building-a-unified-booking-page)
7. [Handling Booking Success](#handling-booking-success)
...
A professional booking system is essential for any portfolio or business website. It streamlines the process of scheduling meetings, reduces back-and-forth emails, and presents a professional image to potential clients.

In this tutorial, we'll implement both Cal.com and Calendly integrations, giving users the flexibility to choose their preferred platform. We'll build:

- Modal-based booking widgets
...
## Choosing Between Cal.com and Calendly
...
- Extensive integrations (Zoom, Teams, etc.)
...
We'll implement both options, letting users choose based on their preferences. This also provides fallback options if one service experiences issues.
...
3. Create event types (e.g., "30-minute meeting", "Discovery call")
4. Get your username (e.g., `leroysteding`)

### Step 2: Install Cal.com Embed
...
### Step 4: Create Cal.com Button Component

Create `components/ui/CalcomButton.tsx`:
...
export default function CalcomButton({ 
  calLink, 
  buttonText = 'Schedule with Cal.com',
  className = ''
}: CalcomButtonProps) {
...
### Step 5: Create Cal.com Modal Component

Create `components/ui/CalcomModal.tsx`:
  ...
  useEffect(() => {
    ...
    (async function initCal() {
      try {
        ...
        setError('Failed to load booking calendar. Please try again.');
        setIsLoading(false);
      }
    })();
  }, [isOpen]);
...
### Step 2: Install Calendly Widget

```bash
pnpm add react-calendly
```

### Step 3: Create Calendly Button Component
...
export default function CalendlyButton({
  url,
  buttonText = 'Schedule with Calendly',
  className = '',
  prefill,
  utm,
}: CalendlyButtonProps) {
...
### Step 4: Create Calendly Modal Component

Create `components/ui/CalendlyModal.tsx`:
...
## Creating Reusable Booking Components

### Unified Booking Card

Create `components/ui/BookingCard.tsx`:
...
## Building a Unified Booking Page
...
Create `app/[locale]/book/BookingPageClient.tsx`:
      ...
      <div className="flex justify-center gap-2 mb-8">
        {(['all', 'calcom', 'calendly'] as const).map((tab) => (
          <button
            ...
            `}
          >
            {tab === 'all' ? 'All' : tab === 'calcom' ? 'Cal.com' : 'Calendly'}
          </button>
        ))}
      </div>
...
## Handling Booking Success

Create `app/[locale]/book/success/page.tsx`:
...
Create `app/[locale]/book/success/BookingSuccessClient.tsx`:
...
Create `lib/analytics/booking.ts`:
...
```typescript
// Detect mobile for full-screen modal
...
```typescript
// Check if Cal is properly initialized
useEffect(() => {
  ...
  setTimeout(() => {
    clearInterval(checkCal);
    if (!window.Cal?.loaded) {
      setError('Calendar failed to load. Please try again.');
    }
  }, 10000);
...
```typescript
// Ensure you're using the correct event names
useCalendlyEventListener({
  onProfilePageViewed: () => console.log('Profile viewed'),
  onDateAndTimeSelected: () => console.log('Time selected'),
  onEventTypeViewed: () => console.log('Event type viewed'),
  onEventScheduled: (e) => console.log('Scheduled!', e),
});
```
...
```typescript
// Add to types/calendar.d.ts if needed
...
1. **A/B Testing**: Test which booking provider converts better
2. **Automation**: Set up Zapier/Make integrations for follow-ups
3. **CRM Integration**: Connect bookings to your CRM
4. **Notifications**: Add Slack/email notifications for new bookings
