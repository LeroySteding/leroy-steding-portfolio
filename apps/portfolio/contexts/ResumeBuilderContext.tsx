...
import { ResumeBuilderState, ResumeCustomization, AISuggestion } from "@/lib/cv/types";
...
export function ResumeBuilderProvider({
  ...
  const requestAISuggestions = useCallback(async (field: string, content: string) => {
    try {
  ...
  const applyAISuggestion = useCallback((field: string, suggestion: string) => {
