...
export const syncIssueToTask = internalAction({
  ...
  handler: async (ctx, args): Promise<{ action: string; taskId: Id<"agent_tasks"> }> => {
...
export const updateLinearIssueStatus = internalAction({
  ...
  handler: async (ctx, args): Promise<{ success: boolean; newState: string }> => {
    const linear = getLinearClient();
...
export const createLinearIssue = internalAction({
  ...
  handler: async (ctx, args): Promise<{ linearIssueId: string; url: string }> => {
    const linear = getLinearClient();
