 ...
 */
function calculateTechStackScore(jobTechnologies, preferences) {
 ...
 */
function calculateLocationScore(jobLocation, jobRemote, preferences) {
    const flags = [];
    let score = 0;
    ...
    const locationMatch = preferences.preferredLocations.some((loc) => normalizedJobLocation.includes(loc.toLowerCase()) || loc.toLowerCase().includes(normalizedJobLocation));
    if (locationMatch) {
        score = 20;
 ...
 */
function calculateSalaryScore(jobSalary, preferences) {
    const flags = [];
 ...
 */
function calculateCompanyScore(jobCompany, preferences) {
    const flags = [];
 ...
 */
function calculateKeywordScore(jobTitle, jobDescription, preferences) {
    const flags = [];
    const matched = [];
    ...
    if (preferences.preferredTitles.length > 0) {
        const titleMatch = preferences.preferredTitles.some((title) => jobTitle.toLowerCase().includes(title.toLowerCase()));
        if (titleMatch) {
 ...
 */
function calculateExperienceScore(jobExperienceLevel, preferences) {
    const flags = [];
    ...
    const match = preferences.targetExperienceLevel.some((level) => normalizedLevel.includes(level.toLowerCase()));
    if (match) {
 ...
 */
export function calculateJobMatchScore(job, preferences) {
...
export const saveUserPreferences = mutation({
    args: {
        ...
        remotePreference: v.optional(v.union(v.literal("required"), v.literal("preferred"), v.literal("acceptable"), v.literal("no_preference"))),
        minSalary: v.optional(v.number()),
        maxSalary: v.optional(v.number()),
        salaryCurrency: v.optional(v.string()),
        techStackPreferences: v.optional(v.array(v.object({
            skill: v.string(),
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            ...
            .withIndex("by_user_id", (q) => q.eq("userId", userId))
            .first();
        const updates = { ...args, userId, updatedAt: now };
...
export const initializeProfileFromSkills = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const techStackPreferences = skills
            ...
            importance: (skill.proficiency >= 80 ? "required" : skill.proficiency >= 65 ? "preferred" : "nice_to_have"),
        }));
