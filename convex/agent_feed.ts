...
const feedType = v.union(
  v.literal("news"), v.literal("trend"), v.literal("alert"),
  v.literal("task_update"), v.literal("deploy"), v.literal("pr"),
  v.literal("briefing"), v.literal("insight")
);
...
export const list: any = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.unreadOnly) items = items.filter((i) => !i.read);
