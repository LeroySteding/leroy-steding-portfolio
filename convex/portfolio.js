...
export const getFeaturedProjects = query({
    ...
    handler: async (ctx, args) => {
        let projects = await ctx.db.query("projects").collect();
        projects = projects.filter((p) => p.featured === true && p.published !== false);
        if (args.locale) {
            projects = projects.filter((p) => p.locale === args.locale);
        }
...
export const getProjectBySlug = query({
    ...
    handler: async (ctx, args) => {
        const projects = await ctx.db
            ...
            .collect();
        if (args.locale) {
            return (projects.find((p) => p.locale === args.locale) ??
...
export const getExperienceBySlug = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.locale) {
            return (matching.find((e) => e.locale === args.locale) ??
...
export const getFeaturedPosts = query({
    ...
    handler: async (ctx, args) => {
        let posts = await ctx.db.query("blog_posts").collect();
        posts = posts.filter((p) => p.featured === true && p.status === "published");
        if (args.locale) {
            posts = posts.filter((p) => p.locale === args.locale);
        }
        return posts.sort((a, b) => {
...
export const getPostBySlug = query({
    ...
    handler: async (ctx, args) => {
        const posts = await ctx.db
            ...
            .collect();
        if (args.locale) {
            return (posts.find((p) => p.locale === args.locale) ??
