...
export const listJobs = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.techStack && args.techStack.length > 0) {
      jobs = jobs.filter((j) =>
        args.techStack!.some((tech) =>
          j.technologies.some((t) => 
    ...
    if (args.minSalary || args.maxSalary) {
      jobs = jobs.filter((j) => {
        ...
        if (args.minSalary && minSalary < args.minSalary) return false;
        if (args.maxSalary && minSalary > args.maxSalary) return false;
        return true;
      });
    }
...
export const weeklyPerformance = query({
  ...
  handler: async (ctx) => {
    ...
    // Group by day
    const dayData: Record<string, { date: string; applied: number; responses: number }> = {};
...
export const scraperHealth = query({
  ...
  handler: async (ctx) => {
    ...
    const errors = errorLogs.filter(
      (log) => log.event.includes("error") || log.event === "prolinker_scraper_fatal_error"
    );
