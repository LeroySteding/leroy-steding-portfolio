...
function calculateLeadScore(lead: any): number {
  ...
  // Source scoring
  if (lead.source) {
    score += SCORING_CONFIG.sourceScores[lead.source as keyof typeof SCORING_CONFIG.sourceScores] || 0;
  }

  // Engagement indicators
  if (lead.name?.trim()) score += SCORING_CONFIG.hasName;
  if (lead.company?.trim()) score += SCORING_CONFIG.hasCompany;
  if (lead.phone?.trim()) score += SCORING_CONFIG.hasPhone;
  if (lead.message?.trim()) score += SCORING_CONFIG.hasMessage;
  if (lead.subscribedToNewsletter) score += SCORING_CONFIG.subscribedToNewsletter;
  ...
  // Context signals
  if (lead.utmSource) score += SCORING_CONFIG.hasUtmSource;
  if (lead.referrer?.includes("linkedin")) score += SCORING_CONFIG.linkedinReferrer;
  if (!lead.referrer) score += SCORING_CONFIG.directVisit;
  ...
  if (lead.metadata) {
    ...
    if (typeof lead.metadata.message_count === "number") {
      if (lead.metadata.message_count >= 5) score += 5;
      else if (lead.metadata.message_count >= 3) score += 3;
    }
  }
    ...
    if (existing) {
      ...
      const mergedMetadata = {
        ...
        previous_interactions: [
          ...((existing.metadata?.previous_interactions as any[]) || []),
          {
            source: args.source,
            timestamp: Date.now(),
