...
export const log = mutation({
  ...
  handler: async (ctx, args) => {
    return await ctx.db.insert("analytics_log", { ...args, createdAt: Date.now() });
  },
});
...
export const list = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.event) items = items.filter((i) => i.event === args.event);
    if (args.agent) items = items.filter((i) => i.agent === args.agent);
    if (args.model) items = items.filter((i) => i.model === args.model);
...
export const summary = query({
  ...
  handler: async (ctx) => {
    ...
    for (const item of all) {
      ...
      if (item.agent) byAgent[item.agent] = (byAgent[item.agent] ?? 0) + 1;
      if (item.model) byModel[item.model] = (byModel[item.model] ?? 0) + 1;
    }
